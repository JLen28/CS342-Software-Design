...
public class BaccaratGameLogic {
	...
	public static int handTotal(ArrayList<Card> hand) {
		if(hand.size() == 3){ // still need to find the total of a 3 card hand
			  
			int val1 = hand.get(0).getCardValue();
			int val2 = hand.get(1).getCardValue(); // 3 cards so 3 values...
			int val3 = hand.get(2).getCardValue();
		...
		} // if we got here, its just a normal 2 card hand, evaluate as usual...
		else {
		int bal1 = hand.get(0).getCardValue();
		int bal2 = hand.get(1).getCardValue();
		int total = bal1 + bal2;
		if(total >= 10) { // total is greater than ten, min possible is 10 and max possible is 18, both can be reduced to single
			return total - 10; // digit by subtracting a base of ten
		}
